...
import { createVoiceSessionSchema, validateRequestBody } from "../_shared/validation.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const validation = await validateRequestBody(req, createVoiceSessionSchema);
    ...
    const { title, description, employeeId } = validation.data;
